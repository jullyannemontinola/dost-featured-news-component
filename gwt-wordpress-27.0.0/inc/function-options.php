...
public function __construct()
{
    ...
    // Check if $this->options is an array and has elements before merging
    if (is_array($this->options) && sizeof($this->options) > 0) {
        $this->options = array_merge($this->_default_options, $this->options);
    } else {
        // If $this->options is not an array or doesn't have elements, use only default options
...
<?php
  ...
  <?php
	...
	{
		$govph_custom_headings_size = isset($this->options['govph_custom_headings_size']) ? $this->options['govph_custom_headings_size'] : 1.0;
		$sm = ($govph_custom_headings_size == 0.8 ? "selected" : "");
		$nl = ($govph_custom_headings_size == 1 ? "selected" : "");
		$lr = ($govph_custom_headings_size == 1.6 ? "selected" : "");
  ?>
  ...
  //Temporarily disabled the input field 
  //requested by Ms. JMS
  public function govph_acc_link_search()
  {
  ?>
    <span class="field-prefix"><?php echo get_site_url(); ?>/ </span>
    <input type="text" name="govph_options[govph_acc_link_search]" value="<?php echo $this->options['govph_acc_link_search'] ?>" readonly placeholder="input disabled"><br/>
...
function govph_displayoptions( $options ){
  ...
  switch ($options) {
    ...
    case 'govph_logo_setting':
      ...
      case 'govph_logo':
        if (isset($option['govph_logo'])) {
            $logo_image = $option['govph_logo'];
        } else {
            $logo_image = get_template_directory_uri().'/images/logo-masthead-large.png';
        }
    ...
    case 'govph_slider_setting':
      ...
      // Check if the key 'govph_slider_fullwidth' exists before accessing it
